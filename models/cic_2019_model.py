...
class Cic_2019_1D(nn.Module):
    def __init__(self, input_channels, num_classes):
        super(Cic_2019_1D, self).__init__()
        ...
        self.dct_model_u6 = DCT_Attention(8)

        self.up7 = nn.ConvTranspose1d(256, 128, kernel_size=2, stride=2)
        self.dct_model_u7 = DCT_Attention(16)
        self.dec7 = self.conv_block(128, 128)

        self.dct_model_u8 = DCT_Attention(32)
        ...
        self.dct_model_u9 = DCT_Attention(64)
    ...
    def forward(self, x):
        ...
        u6 = self.up6(bn)
        u6_residual = e4[:, :, :8]
        u6 = self.dct_model_u6(u6[:, :, :8])
        u6 = u6 + u6_residual
        d6 = self.dec6(u6)

        u7 = self.up7(d6)
        u7_residual = e3[:, :, :16]
        u7 = self.dct_model_u7(u7[:, :, :16])
        u7 = u7 + u7_residual
        d7 = self.dec7(u7)

        u8 = self.up8(d7)
        u8_residual = e2[:, :, :32]
        u8 = self.dct_model_u8(u8[:, :, :32])
        u8 = u8 + u8_residual
